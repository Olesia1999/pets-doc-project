...
Пользователь -[#EF6C00]-> Приложение: Нажимает «Редактировать»
Приложение -[#EF6C00]-> Пользователь: Открывает форму редактирования

Пользователь -[#EF6C00]-> Приложение: Изменяет данные\nНажимает «Сохранить»
Приложение -[#EF6C00]-> Бэк: PUT /notes/{id}
Бэк -[#EF6C00]-> БД: UPDATE note
БД -[#EF6C00]-> Бэк: OK
Бэк -[#EF6C00]-> Приложение: 200 OK
Приложение -[#EF6C00]-> Пользователь: Уведомление\n«Заметка обновлена»

alt Альтернативный сценарий: отмена редактирования
    Пользователь -[#C62828]-> Приложение: <color:red>Нажимает «Отменить»</color>
    Приложение -[#C62828]-> Пользователь: Закрывает форму\nВозврат к списку заметок
end
